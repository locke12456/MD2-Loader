...
extern Vector3	BoxMaxs; //not in use for right now 
...
Cheater::Cheater(Vector3 xyz)
{
	...
	mAirFraction = .5;
	mSpeed = 125; //number of units per second to travel 
	xRadians = 0; //cam must start facing horizontally !
}

void	Cheater::RotateView(float  angle, Vector3 * VecToRotAbout) 
{
	Vector3 NewView;
...
bool	Cheater::CheckMouse() 
{
	...
	rotation.y=rotation.y+(YDegRad*180.0f/PI);
...
void	Cheater::MoveCheater(Vector3	*to)
{
