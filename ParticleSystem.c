...
int Init ( ESContext *esContext )
{
   ...
   for ( i = 0; i < NUM_PARTICLES; i++ )
   {
	  ...
	  int j=i%360;
	  float theta=(float)j/360.0f*360.0f;
	  double PI = 3.14159265;
      // Lifetime of particle
      (*particleData++) = ( (float)(rand() % 15000) / 10000.0f );

      // End position of particle
      (*particleData++) = cos(theta*PI/180.0f)*0.25f;//( (float)(rand() % 10000) / 5000.0f ) - 1.0f;
      (*particleData++) = sin(theta*PI/180.0f)*0.25f;//( (float)(rand() % 10000) / 5000.0f ) - 1.0f;
      (*particleData++) = tan(theta*PI/180.0f)*0.25f;//( (float)(rand() % 10000) / 5000.0f ) - 1.0f;
...
void Update ( ESContext *esContext, float deltaTime )
{
   ...
   if ( userData->time >= 1.0f )
   {
      ...
      // Random color
      color[0] = 0.1f;//( (float)(rand() % 10000) / 20000.0f ) + 0.5f;
      color[1] = 0.1f;//( (float)(rand() % 10000) / 20000.0f ) + 0.5f;
      color[2] = 0.4f;//( (float)(rand() % 10000) / 20000.0f ) + 0.5f;
      color[3] = 0.5;
...
//
//   esCreateWindow ( &esContext, "ParticleSystem", 500, 500, ES_WINDOW_RGB );
//   
//   if ( !Init ( &esContext ) )
//      return 0;
//
